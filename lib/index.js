...
const scrapeFactory = (scrapeDefinition) => {

  const scrapeSingle = async (baseURL, options) => {
  ...
  const scraper = scrapeSingle;
  if (scrapeDefinition.followNext) {
    // TODO implement 
  }
  return {
    scrape: scraper
...
class Rattler {
  ...
  extract() {
    ...
    const scrapePromises = scrapeList.map(
      scrapeDefinition => scrapeFactory(scrapeDefinition).scrape(baseURL, options)
    );
